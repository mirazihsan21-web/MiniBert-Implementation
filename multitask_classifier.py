...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # freeze or fine-tune BERT
        for param in self.bert.parameters():
            param.requires_grad = (config.option == 'finetune')

        # sentiment classifier: 5 classes
        self.sentiment_classifier = nn.Linear(768, 5)

        # paraphrase detector: binary (logit)
        self.paraphrase_classifier = nn.Linear(768 * 2, 1)

        # similarity scorer: single scalar
        self.similarity_regressor = nn.Linear(768 * 2, 1)

        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids, attention_mask):
        '''
        Basic forward: return the [CLS] embedding.
        '''
    ...
    def predict_sentiment(self, input_ids, attention_mask):
        '''
        Predict sentiment class logits (batch_size, 5)
        '''
    ...
    def predict_paraphrase(self,
        ...
        '''
        Predict paraphrase logit (batch_size,)
        '''
    ...
    def predict_similarity(self,
                           ...
                           input_ids_2, attention_mask_2):
        '''
        Predict similarity score (batch_size,)
        '''
        cls_1 = self.forward(input_ids_1, attention_mask_1)
        cls_2 = self.forward(input_ids_2, attention_mask_2)
        combined = torch.cat([cls_1, cls_2], dim=-1)
        score = self.similarity_regressor(combined)
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        print(f"▶️ Epoch {epoch} starting …")
        model.train()
        train_loss = 0
        num_batches = 0
